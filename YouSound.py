...
# If the URL can't be used for creation of YouTube object, it prompts user to enter correct URL.
    def getobject(self):
        try:
            if self.video == "stop":
                exit()
...
"""
Checks if there already is a directory for specified artist, if not, 
creates it and makes it download directory for the song
If there is no author, check if there is NONAME directory, if not, 
crates it and makes it download directory for the song
"""
...
def catalog():
    ...
    if "-" in ytobject.video.title:
        ...
        if os.path.exists(str(os.path.join(directory,  ytobject.author))):
            save_dir = str(os.path.join(directory, ytobject.author))

        elif os.path.exists(str(os.path.join(directory, ytobject.author.upper()))):
            save_dir = str(os.path.join(directory, ytobject.author.upper()))

        elif os.path.exists(str(os.path.join(directory, ytobject.author.lower()))):
            save_dir = str(os.path.join(directory, ytobject.author.lower()))

        elif os.path.exists(str(os.path.join(directory,  ytobject.author.capitalize()))):
            save_dir = str(os.path.join(directory, ytobject.author.capitalize()))
        ...
        if os.path.exists(str(os.path.join(directory,  "NONAME"))):
            save_dir = str(os.path.join(directory, "NONAME"))
        else:
...
def download():
    ...
    stream.download(save_directory)
...
def convert():
    ...
    commandconv = "ffmpeg -i \"" + os.path.join(save_directory, re.sub("\.", "" , ytobject.video.title)) + "\".mp4 -ab 160k -ac 2 -ar 44100 -vn \"" + os.path.join(save_directory, ytobject.title) + "\".mp3"
    commandrm = "rm \"" + os.path.join(save_directory, re.sub("\.", "" , ytobject.video.title)) + "\".mp4"
